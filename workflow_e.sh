...
useC=$6	#Use C implementation or Perl? INPUT IS T to use C, or F to use default perl
...
if [[ -e $groups_fa_file ]]; then
	...
	if [ -z "$useC" ]; then #Check if flag has been set 
		$BIN/group_gt1.pl $rekeyedtabbedfile $name 	# PERL implementation is default
	else
		if [ $useC == "T" ];	then
			# REP this with the C implementation ASK G to CHANGE DIRECTORY 
			../testDir/group_gt1 $rekeyedtabbedfile $name # Use C version
		else
			$BIN/group_gt1.pl $rekeyedtabbedfile $name 	# Choose to use PERL
		fi
	fi
